...
object Grid {
  ...
  def parseGrid() {
    // test a two dimensional array
    var x = 0
    var y = 0
    var highestProduct = 0l
    var tempProduct = 0l

    val maxLength = arr.length - 4
    /*
    for (x <- 0 to maxLength) {
      for (y <- 0 to maxLength) {
        println("tempdiag for: " + x + ", " + y)
        val tempList = getDiagonal(x,y)
        println(tempList)
        tempProduct = tempList.foldLeft(1)(_*_)
    ...
    */
    ...
    for (x <- 0 to maxLength) {
      for (y <- 0 to arr.length - 1) {
        println("temphoriz for: " + x + ", " + y)
        val tempList = getHorizontal(x,y)
        println(tempList)
        tempProduct = tempList.foldLeft(1)(_*_)
        if (tempProduct > highestProduct) {
          highestProduct = tempProduct
   ...
   * @param startX
   * @param startY
   * this should always be the top-left starting point
   * don't need lists, we can just calculate the product right in place.....
   */
  def getDiagonal(startX: Int, startY: Int): List[Int] = {
    if (startX < arr.length - 3 && startY < arr.length - 3)
      List(arr(startY)(startX), arr(startY + 1)(startX + 1), arr(startY + 2)(startX + 2), arr(startY + 3)(startX + 3))
    else
      List(1)
  }

  def getDiagonal2(startX: Int, startY: Int): List[Int] = {
    println("diagonal2 coords: " + startX + ", " + startY)
    if (startX < arr.length - 3 && startY > 2)
      List(arr(startY)(startX), arr(startY - 1)(startX + 1), arr(startY - 2)(startX + 2), arr(startY - 3)(startX + 3))
    else
      List(1)
  }

  def getHorizontal(startX: Int, startY: Int): List[Int] = {
    if (startX < arr.length - 3)
      List(arr(startY)(startX), arr(startY)(startX + 1), arr(startY)(startX + 2), arr(startY)(startX + 3))
    else
      null
  }

  def getVertical(startX: Int, startY: Int): List[Int] = {
    if (startY < arr.length - 3)
      List(arr(startY)(startX), arr(startY + 1)(startX), arr(startY + 2)(startX), arr(startY + 3)(startX))
    else
      null
  }
...
for (int i = 2; i <= k; i++) {
if (prime[i]) sum = sum.add(BigInteger.valueOf(i));
}
return sum;
}
